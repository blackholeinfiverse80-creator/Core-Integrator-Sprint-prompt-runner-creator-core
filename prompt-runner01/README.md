...
- Returns the strict instruction shape: `prompt`, `module`, `intent`, `topic`, `tasks`, `output_format`, `product_context`.
...
- POST `/generate` — Generate the instruction
  ...
  - Response (JSON):
    ...
    - `product_context` (string, fixed to `creator_core`)
...
```json
{
  "prompt": "Create a workflow to manage early stage diabetes patients",
  "module": "workflow",
...
### Optional downstream publisher

`PlatformAdapter.process(..., publish=True)` now supports secure delivery to downstream consumers.

Environment variables:

- `CONSUMER_URL` (required for publish)
- `CONSUMER_API_KEY` (required for publish, sent as Bearer token)
- `PUBLISH_MAX_ATTEMPTS` (optional, default `3`)
- `PUBLISH_BACKOFF_BASE_SECONDS` (optional, default `0.5`)
- `PUBLISH_TIMEOUT_SECONDS` (optional, default `10`)
- `PUBLISH_DLQ_DIR` (optional, default `dlq`)

Delivery behavior:

- Validates instruction against schema before sending.
- Adds idempotency and tracing headers (`Idempotency-Key`, `X-Correlation-ID`, `X-Instruction-ID`).
- Retries with exponential backoff.
- Writes failed envelopes to DLQ JSON files on terminal failure.
